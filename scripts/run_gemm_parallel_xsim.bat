@echo off
REM D3: compile and run parallel GEMM python-vector regression.
REM Usage: scripts\run_gemm_parallel_xsim.bat
...
call xvlog --incr --relax -sv -work xil_defaultlib ^
  ...
  glbl.v -log xvlog_gemm_parallel_py.log
if errorlevel 1 exit /b 1
...
call xelab --incr --debug typical --relax --mt 2 -L xil_defaultlib -L unisims_ver -L unimacro_ver -L secureip ^
  ...
  xil_defaultlib.tb_gemm_parallel_python_vectors xil_defaultlib.glbl -log xelab_gemm_parallel_py.log
if errorlevel 1 exit /b 1

call xsim tb_gemm_parallel_python_vectors_behav -log simulate_gemm_parallel_py.log -R
exit /b %errorlevel%
