@echo off
REM D7: compile and run tiled QKV multi-head regression.
REM Usage: scripts\run_tiled_qkv_xsim.bat

call F:\vivado\2026.1\Vivado\settings64.bat
cd /d F:\Users\22563\Desktop\FPGAlearn\requantize_int8_rtl

set KMP_DUPLICATE_LIB_OK=TRUE
python scripts\generate_linear_tiled_qkv_vectors.py
...
call xvlog --incr --relax -sv -work xil_defaultlib ^
  ...
  %SRC%/sim_1/new/tb_linear_tiled_qkv.sv ^
  glbl.v -log xvlog_tiled_qkv.log
if errorlevel 1 exit /b 1

call xelab --incr --debug typical --relax --mt 2 -L xil_defaultlib -L unisims_ver -L unimacro_ver -L secureip ^
  --snapshot tb_linear_tiled_qkv_behav ^
  xil_defaultlib.tb_linear_tiled_qkv xil_defaultlib.glbl -log xelab_tiled_qkv.log
if errorlevel 1 exit /b 1

call xsim tb_linear_tiled_qkv_behav -log simulate_tiled_qkv.log -R
exit /b %errorlevel%
