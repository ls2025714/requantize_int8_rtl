...
module tb_linear_tiled_qkv;
    ...
    localparam int WEIGHT_ELEMS        = NUM_OPS * NUM_HEADS * FULL_N * FULL_K;
    localparam int MULT_ELEMS          = NUM_OPS * NUM_HEADS * FULL_N;
    ...
    localparam int EXPECTED_NUM_CASES  = 5;
    localparam int EXPECTED_SEED       = 20260831;
    localparam string VECTOR_FILE      =
        "F:/Users/22563/Desktop/FPGAlearn/requantize_int8_rtl/tiled_qkv_vectors.txt";
    ...
    task automatic send_command(
        ...
        begin
            ...
            cmd_op_type = op_value[1:0];
            cmd_head_idx = head_value[1:0];
            cmd_valid = 1'b1;
    ...
    task automatic read_mult_block(
        input integer expected_count,
        output integer values [0:MULT_ELEMS-1],
        output integer actual_count,
        output string next_line
    );
        integer scan_count;
        ...
        begin
            actual_count = 0;
            next_line = "";
            while (!$feof(vector_file) && (actual_count < expected_count)) begin
                if ($fgets(line_str, vector_file) == 0) begin
                    continue;
                end
                if (line_str.len() == 0) begin
                ...
                if (is_section_marker(line_str) || (line_str.substr(0, 3) == "CASE")) begin
                    next_line = line_str;
                    break;
                end
                scan_count = $sscanf(line_str, "%d", val);
                if (scan_count != 1) begin
                    mismatch_count = mismatch_count + 1;
                    $display("FAIL parse PRELOAD_MULT line=%s", line_str);
                    return;
                end
                values[actual_count] = val;
                actual_count = actual_count + 1;
            ...
            if (actual_count != expected_count) begin
                mismatch_count = mismatch_count + 1;
                $display(
                    "FAIL PRELOAD_MULT count got=%0d expected=%0d",
    ...
    task automatic load_banks;
        ...
        integer mult_vals [0:MULT_ELEMS-1];
        string marker_line;
        begin
            read_int_block(WEIGHT_ELEMS, w_vals, w_count, marker_line, 0, "PRELOAD_WEIGHT");
            if (w_count != WEIGHT_ELEMS) begin
                $display("ERROR: PRELOAD_WEIGHT incomplete");
                $fatal;
            end
            if (marker_line != "PRELOAD_MULT\n") begin
                $display("ERROR: missing PRELOAD_MULT marker got=%s", marker_line);
                $fatal;
            end
            read_mult_block(MULT_ELEMS, mult_vals, mult_count, marker_line);
            if (mult_count != MULT_ELEMS) begin
                $display("ERROR: PRELOAD_MULT incomplete");
                $fatal;
            end
            ...
            banks_loaded = 1'b1;
            $display("INFO: preloaded WEIGHT=%0d MULT=%0d", WEIGHT_ELEMS, MULT_ELEMS);
        end
    endtask
    ...
    task automatic run_vector_case(
        ...
        integer base_addr;
        begin
            base_addr = op_value * (NUM_HEADS * FULL_N * FULL_K) + head_value * (FULL_N * FULL_K);
            $display(
                "CASE %0d op=%0d head=%0d M=%0d base_addr=%0d",
                case_id, op_value, head_value, m_value, base_addr
            );
            ...
            send_command(m_value, op_value, head_value);
    ...
    initial begin
        ...
        $display("D7 Tiled QKV multi-head regression");
        ...
        while (!$feof(vector_file)) begin
            ...
            if (line_str == "PRELOAD_WEIGHT\n") begin
                load_banks();
                continue;
            end
            if (line_str.substr(0, 3) == "CASE") begin
                if (!banks_loaded) begin
                    mismatch_count = mismatch_count + 1;
                    $display("FAIL CASE before PRELOAD");
                    continue;
                end
                scan_count = $sscanf(
                    line_str,
                    "CASE %d %d %d %d %d %d",
                    case_id, op_value, head_value, m_value, a_gap, c_hold
                );
                ...
                run_vector_case(case_id, op_value, head_value, m_value, a_gap, c_hold);
            end
        end
